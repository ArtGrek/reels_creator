...
pub async fn execute(a_game_name: String, a_location: String) {
    ...
    let symbols: Vec<i64> = game_config.get("symbols").and_then(|v| v.as_array()).unwrap_or(&vec![]).iter().filter_map(|v| v.as_i64()).collect();
    let coin: i64 = game_config.get("coin").and_then(|v| v.as_i64()).unwrap_or(10);
    let specials: Vec<i64> = game_config.get("specials").and_then(|v| v.as_array()).unwrap_or(&vec![]).iter().filter_map(|v| v.as_i64()).collect();
    let mysterty: i64 = game_config.get("mysterty").and_then(|v| v.as_i64()).unwrap_or(15);
    ...
    extract(&transactions, &mut game, &spins_symbols, &specials, &bonus_symbols, &bonus_symbol_values, mysterty, buy_count, width, height, );
