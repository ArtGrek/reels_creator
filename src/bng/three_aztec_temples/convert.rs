...
use crate::bng::three_aztec_temples::models::Categories;

pub fn _save_fugaso (a_categories: Categories, a_appearing_symbols: Vec<i64>, a_location: String, a_game_name: String) {
    let format_reels = format!("{{\n\t\"stopFactor\": {},\n\t\"betCounters\": {},\n\t\"distBuy\": {},\n\t\"distRespin\": {},\n\t\"distCoin\": {},\n\t\"distBaseCategory\": {},\n\t\"distSymbol\": [{}, 10000],\n\t\"distMystery\": {{\n{}\n\t}},\n\t\"distBombs\": [{}],\n\t\"distBonus\": {},\n\t\"distNewCoin\": [{}],\n\t\"reels\": [\n{}\n\t],\n\t\"reelsBonus\": {{\n{}\n\t}},\n\t\"lines\": {},\n\t\"wins\": {}\n}}",
        //stopFactor
        10000,
        //betCounters
        {
            let p1 = a_categories.settings.get("buy_bonus_price").and_then(|v| v.as_array()).and_then(|arr| arr.get(0)).and_then(Value::as_i64).unwrap();
            let p2 = a_categories.settings.get("buy_bonus_price").and_then(|v| v.as_array()).and_then(|arr| arr.get(1)).and_then(Value::as_i64).unwrap();
            let json = format!("[\n\t\t{},\n\t\t{},\n\t\t{}\n\t]", 1, p1, p2);
            json
        },
        //distBuy
        {
            ...
            for cat in &a_categories.buy_category[0] {
                for mech in &cat.bonus.by_mechanics {
                    let name = _mechanics_name(mech.id.clone());
        ...
        //distRespin
        {
            ...
            for cat in &a_categories.category {
                for mech in &cat.bonus.by_mechanics {
                    *sum_inits
                        .entry(_mechanics_name(mech.id.clone()))
        ...
        //distMystery
        {
            ...
            a_categories.category[0].bonus.mystery_symbols.iter().map(|sym| {
                ...
                format!("\t\t\"{:.0}\": \"{}\"", p, _num_str_to_letter(&sym.id.to_string()).unwrap())
        ...
        //distBonus
        {
            ...
            for (mech_id, (total_reap, combo_map)) in agg {
                ...
                let mech_id_str = _mechanics_name(mech_id);
                ...
                for (combo_key, combo_val) in combo_map {
                    ...
                    if total_reap != 0 {
                        ...
                        let combo_key_str = combo_key
                            .iter()
                            .filter_map(|key| _num_str_to_letter(&key.to_string()))
                            .collect::<Vec<_>>()
                            .join("");
        ...
        //distNewCoin
        {
            ...
            for mech_id in mech_ids {
                ...
                mech_blocks1.push(format!("\t\t\"{}\":[\n{}\n\t\t]", _mechanics_str_name(&mech_id), len_blocks.join(",\n")));
            }
                    ...
                    category.reels.iter().map(|reels| {
                        format!("\t\t\t\"{}\"", 
                            reels.instanses.iter().filter(|instanse| {instanse.correct}).last().map(|r| {
                                ...
                                r.reel.iter().take(count).map(|x| format!("{}", _num_str_to_letter(&x.to_string()).unwrap_or("Z".to_string()))).collect::<Vec<_>>().join("")
                            }).unwrap_or_default()
                        )
                    }).collect::<Vec<String>>().join(",\n")
                )
                ...
                for mech_id in mech_ids {
                    ...
                    for la in lengths.iter() {
                        ...
                        for sid in sids {
                            ...
                            let ch = _num_str_to_letter(&new_id.to_string()).and_then(|s| s.chars().next()).unwrap_or('@');
        ...
        //wins
        {
            ...
            if let Value::Object(m) = a_categories.settings.get("paytable").unwrap() {
                for (num_str, arr_val) in m {
                    if let Some(letter) = _num_str_to_letter(num_str) {
...
fn _mechanics_name(mut ids: Vec<i64>) -> String {
    ids.sort();
    ids.dedup();
    ids.iter()
...
fn _mechanics_str_name(ids_str: &str) -> String {
...
fn _num_str_to_letter(s: &str) -> Option<String> {
...
pub fn _fugaso_by_amount(a_transactions: &Vec<Value>, a_location: String, a_game_name: String) {
    let bins = [(0,    1), (1,    5),
