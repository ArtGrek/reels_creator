...
use crate::bng::china_festival::models::Category;
...
fn filter(input: &Value) -> Value {
    ...
    if let Some(ctx) = input.get("out").and_then(|o| o.get("context")).and_then(|v| v.as_object()) {
        ...
        if let Some(bonus_obj) = ctx.get("bonus").and_then(|v| v.as_object()) {
            let bonus_keys = [
                "bet_per_line","lines","bonus_scenario","bonus_mechanic", "bs_count",
                "boost_values","collect_values","double_values",
                "mystery_values","new_bs","orig_board","board","orig_bs_v",
                "round_bet","round_win",
            ];
                ...
                for (len_idx, la) in lengths.iter().enumerate() {
                    if (5 + mech_id.len() - 1) <= len_idx {
                        ...
                        for sid in sids {
                            ...
                            if p_sym > 0.0 {
                                if sid == 13 {sym_entries.push(format!("\t\t\t\t\t\"{:.0}\":{{\"id\":\"{}\"}}",cum_sym_p, sid));}
                                else {sym_entries.push(format!("\t\t\t\t\t\"{:.0}\":{{\"id\":\"{}\",\"values\":{{{}}}}}",cum_sym_p, sid, val_entries.join(",")));}
                            } 
                            else {
                                if mech_id.contains(&(sid-10).to_string()) {
                                    if sid == 13 {sym_entries.push(format!("\t\t\t\t\t\"{:.0}\":{{\"id\":\"{}\"}}",0.0, sid));}
                        ...
                        for sid in mids {
                            ...
                            if sym_prob > 0.0 {
                                if sid == 13 {myst_entries.push(format!("\t\t\t\t\t\"{:.0}\":{{\"id\":\"{}\"}}",cum_sym_p, sid));}
                                else {myst_entries.push(format!("\t\t\t\t\t\"{:.0}\":{{\"id\":\"{}\",\"values\":{{{}}}}}",cum_sym_p, sid, val_entries.join(",")));}
                            } 
                            else {
                                if sid == 13 {myst_entries.push(format!("\t\t\t\t\t\"{:.0}\":{{\"id\":\"{}\"}}",0.0, sid));}
