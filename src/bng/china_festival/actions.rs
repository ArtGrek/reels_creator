...
use crate::bng::china_festival::models::{Categories, Category, Spins, Bet, Win, ByLine, BySymbol, ByLenght, Boards, UniqueBoardsInstanse, BoardsInstanse, Board, Bonus, ByMechanic, ByBonusLenght, Symbol, Col, Row, Reel, ReelInstanse, Multi};
        ...
        if transaction.get("out").and_then(|response| response.get("command")).and_then(|command| command.as_str()) == Some("play") {
            if transaction.get("out").and_then(|response| response.get("status")).and_then(|status| status.get("code")).and_then(|code| code.as_str()) == Some("OK") {
                ...
                if let Some(context) = transaction.get("out").and_then(|response| response.get("context")) {
                    let category_num = context.get("spins").and_then(|spins| spins.get("spin_type")).and_then(|v| v.as_i64()).unwrap();
                    ...
                    if context.get("current").and_then(|v| v.as_str()) == Some("spins") {
                        ...
                        {    
                            ...
                            let round_bet = if l_last_action == Some("spin") {context.get("spins").and_then(|spins| spins.get("round_bet")).and_then(|v| v.as_i64()).unwrap_or_default()}
                                ...
                                if l_selected_mode == Some("1") {a_categories.settings.get("buy_bonus_price_1").and_then(Value::as_i64).unwrap() * a_categories.settings.get("bet_factor").and_then(Value::as_array).and_then(|arr| arr.get(0)).and_then(Value::as_i64).unwrap_or(1)}
                                else if l_selected_mode == Some("2") {a_categories.settings.get("buy_bonus_price_2").and_then(Value::as_i64).unwrap() * a_categories.settings.get("bet_factor").and_then(Value::as_array).and_then(|arr| arr.get(0)).and_then(Value::as_i64).unwrap_or(1)}
                                else {0}
                            } else {0};
                    ...
                    //bonus
                        ...
                        if context.get("bonus").and_then(|bonus| bonus.get("bonus_scenario")).and_then(|v| v.as_i64()) == Some(0) {
                            l_category = &mut a_categories.category[category_num as usize];
                        ...
                        
                        {
                            ...
                            let mut orig_board = context.get("bonus").and_then(|bonus| bonus.get("orig_board")).and_then(|orig_board| orig_board.as_array()).map(|array_outer| {
                                array_outer.iter().filter_map(|array_inner| {
                                    array_inner.as_array().map(|value| {value.iter().filter_map(|v| {v.as_i64()}).collect::<Vec<i64>>()})
                            }).collect::<Vec<Vec<i64>>>()}).unwrap_or_default();
                            ...
                            let orig_bs_v = context.get("bonus").and_then(|bonus| bonus.get("orig_bs_v")).and_then(|orig_bs_v| orig_bs_v.as_array()).map(|array_outer| {
                            ...
                            if last_action == "bonus_init" {
                                orig_board = context.get("bonus").and_then(|bonus| bonus.get("board")).and_then(|orig_board| orig_board.as_array()).map(|array_outer| {
                                    array_outer.iter().filter_map(|array_inner| {
                                        array_inner.as_array().map(|value| {value.iter().filter_map(|v| {v.as_i64()}).collect::<Vec<i64>>()})
                                }).collect::<Vec<Vec<i64>>>()}).unwrap_or_default();
                            ...
                            for xy in new_bs.clone() {
                                if let Some(idx) = l_category.bonus.symbols.iter().position(|s| s.id == orig_board[xy[0] as usize][xy[1] as usize]) {
                                    ...
                                    if let Some(value) = l_category.bonus.symbols[idx].values.get_mut(&orig_bs_v[xy[0] as usize][xy[1] as usize]) {*value += 1;}
                                    if l_category.bonus.symbols[idx].id == a_mysterty_symbol {
                                        if let Some(idy) = l_category.bonus.mystery_symbols.iter().position(|s| s.id == board[xy[0] as usize][xy[1] as usize]) {
                                            ...
                                            if let Some(value) = l_category.bonus.mystery_symbols[idy].values.get_mut(&orig_bs_v[xy[0] as usize][xy[1] as usize]) {*value += 1;}
                                        }
                                    }
                                }
                                if let Some(idx) = l_category.bonus.by_bonus_lenghts[current_bs_count-1].symbols.iter().position(|s| s.id == orig_board[xy[0] as usize][xy[1] as usize]) {
                                    ...
                                    if let Some(value) = l_category.bonus.by_bonus_lenghts[current_bs_count-1].symbols[idx].values.get_mut(&orig_bs_v[xy[0] as usize][xy[1] as usize]) {*value += 1;}
                                    if l_category.bonus.by_bonus_lenghts[current_bs_count-1].symbols[idx].id == a_mysterty_symbol {
                                        if let Some(idy) = l_category.bonus.by_bonus_lenghts[current_bs_count-1].mystery_symbols.iter().position(|s| s.id == board[xy[0] as usize][xy[1] as usize]) {
                                            ...
                                            if let Some(value) = l_category.bonus.by_bonus_lenghts[current_bs_count-1].mystery_symbols[idy].values.get_mut(&orig_bs_v[xy[0] as usize][xy[1] as usize]) {*value += 1;}
                                        }
                                    }
                                }
                                match orig_board[xy[0] as usize][xy[1] as usize] {
                                    11 => flags.0 = true,
                                    12 => flags.1 = true,
                                    13 => flags.2 = true,
                                    _  => {}
                            ...
                            if let Some(idm) = l_category.bonus.by_mechanics.iter().position(|mechanic| mechanic.id == current_bonus_mechanic) {
                                for xy in new_bs.clone() {
                                    if let Some(idx) = l_category.bonus.by_mechanics[idm].symbols.iter().position(|s| s.id == orig_board[xy[0] as usize][xy[1] as usize]) {
                                        ...
                                        if let Some(value) = l_category.bonus.by_mechanics[idm].symbols[idx].values.get_mut(&orig_bs_v[xy[0] as usize][xy[1] as usize]) {*value += 1;}
                                        if l_category.bonus.symbols[idx].id == a_mysterty_symbol {
                                            if let Some(idy) = l_category.bonus.by_mechanics[idm].mystery_symbols.iter().position(|s| s.id == board[xy[0] as usize][xy[1] as usize]) {
                                                ...
                                                if let Some(value) = l_category.bonus.by_mechanics[idm].mystery_symbols[idy].values.get_mut(&orig_bs_v[xy[0] as usize][xy[1] as usize]) {*value += 1;}
                                            }
                                        }
                                    }
                                    if let Some(idx) = l_category.bonus.by_mechanics[idm].by_bonus_lenghts[current_bs_count-1].symbols.iter().position(|s| s.id == orig_board[xy[0] as usize][xy[1] as usize]) {
                                        ...
                                        if let Some(value) = l_category.bonus.by_mechanics[idm].by_bonus_lenghts[current_bs_count-1].symbols[idx].values.get_mut(&orig_bs_v[xy[0] as usize][xy[1] as usize]) {*value += 1;}
                                        if l_category.bonus.by_mechanics[idm].by_bonus_lenghts[current_bs_count-1].symbols[idx].id == a_mysterty_symbol {
                                            if let Some(idy) = l_category.bonus.by_mechanics[idm].by_bonus_lenghts[current_bs_count-1].mystery_symbols.iter().position(|s| s.id == board[xy[0] as usize][xy[1] as usize]) {
                                                ...
                                                if let Some(value) = l_category.bonus.by_mechanics[idm].by_bonus_lenghts[current_bs_count-1].mystery_symbols[idy].values.get_mut(&orig_bs_v[xy[0] as usize][xy[1] as usize]) {*value += 1;}
                                            }
                                        }
                                    }
                                    match orig_board[xy[0] as usize][xy[1] as usize] {
...
pub fn set_default_symbols(a_symbols: &Vec<i64>, a_bonus_symbol_values: &Vec<Multi>, width: usize, height: usize) -> Vec<Symbol> {
    ...
    for symbol in a_symbols {
        ...
        symbols.push(Symbol {id: *symbol, count: 0, values: values, cols: cols.clone()});
    };
    symbols
}
...
pub fn collect_reels(a_boards: &Vec<Vec<i64>>, height: usize, can_skip_reel_collect_after_timeout: bool, skip_collect_timeout_sec: u64) -> Vec<ReelInstanse> {
    let pb_main = ProgressBar::new((a_boards.len()) as u64);
...
pub fn check_reels(reels: &mut Vec<ReelInstanse>, boards: &Vec<Vec<i64>>, height: usize/*, cut_last_repeating_chars: bool, save_only_one_instance: bool*/) {
    let pb_main = ProgressBar::new((reels.len()) as u64);
